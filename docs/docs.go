// Package docs Code generated by swaggo/swag. DO NOT EDIT
package docs

import "github.com/swaggo/swag"

const docTemplate = `{
    "schemes": {{ marshal .Schemes }},
    "swagger": "2.0",
    "info": {
        "description": "{{escape .Description}}",
        "title": "{{.Title}}",
        "contact": {},
        "version": "{{.Version}}"
    },
    "host": "{{.Host}}",
    "basePath": "{{.BasePath}}",
    "paths": {
        "/auth/login": {
            "post": {
                "description": "Autentica o usuário e retorna tokens JWT",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Auth"
                ],
                "summary": "Realiza o login do usuário",
                "parameters": [
                    {
                        "description": "Credenciais de login",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.LoginRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.LoginResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/dto.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/dto.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/auth/logout": {
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Invalida a sessão do usuário",
                "tags": [
                    "Auth"
                ],
                "summary": "Realiza o logout",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/auth/me": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna os dados do usuário autenticado",
                "tags": [
                    "Auth"
                ],
                "summary": "Retorna informações do usuário",
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.UsuarioInfo"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/dto.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/auth/refresh": {
            "post": {
                "description": "Usa o refresh token para gerar um novo access token",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Auth"
                ],
                "summary": "Renova o token de acesso",
                "parameters": [
                    {
                        "description": "Refresh token",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.RefreshTokenRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.RefreshTokenResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/dto.ErrorResponse"
                        }
                    },
                    "401": {
                        "description": "Unauthorized",
                        "schema": {
                            "$ref": "#/definitions/dto.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/condicoes-pagamento": {
            "get": {
                "description": "Retorna uma lista paginada de condições de pagamento, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Condições de Pagamento"
                ],
                "summary": "Lista as condições de pagamento",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por tipo de condição (0=À Vista, 1=À Prazo, 9=Sem Pagamento)",
                        "name": "tipo_condicao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.CondicaoPagamentoListResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria uma nova condição de pagamento com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Condições de Pagamento"
                ],
                "summary": "Cria uma nova condição de pagamento",
                "parameters": [
                    {
                        "description": "Dados para criar a condição de pagamento",
                        "name": "condicao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.CondicaoPagamentoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.CondicaoPagamentoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/condicoes-pagamento/{id}": {
            "get": {
                "description": "Retorna os detalhes de uma condição de pagamento específica.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Condições de Pagamento"
                ],
                "summary": "Busca uma condição de pagamento por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Condição de Pagamento",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.CondicaoPagamentoResponse"
                        }
                    },
                    "404": {
                        "description": "Condição de pagamento não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de uma condição de pagamento existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Condições de Pagamento"
                ],
                "summary": "Atualiza uma condição de pagamento",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Condição de Pagamento",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar a condição de pagamento",
                        "name": "condicao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.CondicaoPagamentoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.CondicaoPagamentoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Condição de pagamento não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de uma condição de pagamento.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Condições de Pagamento"
                ],
                "summary": "Exclui uma condição de pagamento",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Condição de Pagamento",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/documentos-venda": {
            "get": {
                "description": "Retorna uma lista paginada de documentos de venda, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Lista os documentos de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por tipo",
                        "name": "tipo",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaListResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria um novo documento de venda com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Cria um novo documento de venda",
                "parameters": [
                    {
                        "description": "Dados para criar o documento de venda",
                        "name": "documento",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/documentos-venda/{documento_id}": {
            "get": {
                "description": "Retorna os detalhes de um documento de venda específico.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Busca um documento de venda por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de um documento de venda existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Atualiza um documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar o documento de venda",
                        "name": "documento",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de um documento de venda.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Exclui um documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/documentos-venda/{documento_id}/cancelar": {
            "post": {
                "description": "Cancela um documento de venda, alterando seu status para cancelado.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Cancela um documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro ao cancelar documento",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "422": {
                        "description": "Documento não pode ser cancelado (validação de negócio)",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/documentos-venda/{documento_id}/emitir": {
            "post": {
                "description": "Emite um documento de venda, alterando seu status para emitido.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Emite um documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro ao emitir documento",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "422": {
                        "description": "Documento não pode ser emitido (validação de negócio)",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/documentos-venda/{documento_id}/itens": {
            "get": {
                "description": "Retorna todos os itens de um documento de venda específico.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Lista os itens de um documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/dto.DocumentoVendaItemResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Documento de venda não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Adiciona um novo item a um documento de venda existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Adiciona um item ao documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados do item",
                        "name": "item",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaItemRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "422": {
                        "description": "Erro de validação de negócio (ex: estoque insuficiente)",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/documentos-venda/{documento_id}/itens/{item}": {
            "put": {
                "description": "Atualiza os dados de um item específico de um documento de venda.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Edita um item do documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do Item",
                        "name": "item",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados atualizados do item",
                        "name": "item",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaItemRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda ou item não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "422": {
                        "description": "Erro de validação de negócio",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Remove um item específico de um documento de venda.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Remove um item do documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do Item",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro ao remover item",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda ou item não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "422": {
                        "description": "Erro de validação de negócio",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/documentos-venda/{documento_id}/pagamentos": {
            "get": {
                "description": "Retorna todos os pagamentos de um documento de venda específico.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Lista os pagamentos de um documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/dto.DocumentoVendaPagamentoResponse"
                            }
                        }
                    },
                    "404": {
                        "description": "Documento de venda não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Adiciona um novo pagamento a um documento de venda existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Adiciona um pagamento ao documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados do pagamento",
                        "name": "pagamento",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaPagamentoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "422": {
                        "description": "Erro de validação de negócio (ex: valor excede o total)",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/documentos-venda/{documento_id}/pagamentos/{pagamento}": {
            "put": {
                "description": "Atualiza os dados de um pagamento específico de um documento de venda.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Documentos de Venda"
                ],
                "summary": "Edita um pagamento do documento de venda",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Documento de Venda",
                        "name": "documento_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do Pagamento",
                        "name": "pagamento",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados atualizados do pagamento",
                        "name": "pagamento",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaPagamentoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.DocumentoVendaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Documento de venda ou pagamento não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "422": {
                        "description": "Erro de validação de negócio",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/empresas-filiais": {
            "get": {
                "description": "Retorna uma lista paginada de filiais de empresa",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Empresa - Filiais"
                ],
                "summary": "Lista todas as filiais de empresa",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Limite de itens por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Número de itens a pular",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EmpresaFilialListResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria uma nova filial de empresa com os dados fornecidos",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Empresa - Filiais"
                ],
                "summary": "Cria uma nova filial de empresa",
                "parameters": [
                    {
                        "description": "Dados da Filial de Empresa",
                        "name": "filial",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EmpresaFilialRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.EmpresaFilialResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/empresas-filiais/{id}": {
            "get": {
                "description": "Retorna uma filial de empresa específica pelo seu ID",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Empresa - Filiais"
                ],
                "summary": "Busca uma filial de empresa pelo ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Filial de Empresa",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EmpresaFilialResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza uma filial de empresa com os dados fornecidos pelo ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Empresa - Filiais"
                ],
                "summary": "Atualiza uma filial de empresa existente",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Filial de Empresa",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados da Filial de Empresa para atualização",
                        "name": "filial",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EmpresaFilialRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EmpresaFilialResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Exclui logicamente uma filial de empresa pelo ID",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Empresa - Filiais"
                ],
                "summary": "Exclui uma filial de empresa",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Filial de Empresa",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/entidades": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna uma lista paginada de entidades",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Entidades"
                ],
                "summary": "Lista entidades",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Limite de registros",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Offset para paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por nome (razao social)",
                        "name": "nome",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por documento",
                        "name": "documento",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por tipo (1-Física, 2-Jurídica)",
                        "name": "tipo_pessoa",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1-Ativo, 2-Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeListResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Cadastra uma nova entidade (cliente/fornecedor)",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Entidades"
                ],
                "summary": "Cria uma nova entidade",
                "parameters": [
                    {
                        "description": "Dados da entidade",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/documento/{documento}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna os dados de uma entidade pelo CPF/CNPJ",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Entidades"
                ],
                "summary": "Busca entidade por documento",
                "parameters": [
                    {
                        "type": "string",
                        "description": "CPF/CNPJ da entidade",
                        "name": "documento",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{entidadeId}/contatos": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna todos os contatos de uma entidade",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeContatos"
                ],
                "summary": "Lista contatos de uma entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Limite de registros",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Offset para paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por tipo de contato",
                        "name": "tipo",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeContatoListResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Cadastra um novo contato para uma entidade",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeContatos"
                ],
                "summary": "Cria um novo contato",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados do contato",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeContatoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeContatoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{entidadeId}/contatos/{item}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna os dados de um contato específico",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeContatos"
                ],
                "summary": "Busca um contato específico",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do contato",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeContatoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Atualiza os dados de um contato existente",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeContatos"
                ],
                "summary": "Atualiza um contato",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do contato",
                        "name": "item",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados atualizados",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeContatoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeContatoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Realiza a exclusão lógica de um contato",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeContatos"
                ],
                "summary": "Exclui um contato",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do contato",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{entidadeId}/documentos": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna todos os documentos de uma entidade",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeDocumentos"
                ],
                "summary": "Lista documentos de uma entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Limite de registros",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Offset para paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por tipo de documento",
                        "name": "tipo",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeDocumentoListResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Cadastra um novo documento para uma entidade",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeDocumentos"
                ],
                "summary": "Cria um novo documento",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados do documento",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeDocumentoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeDocumentoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{entidadeId}/documentos/{item}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna os dados de um documento específico",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeDocumentos"
                ],
                "summary": "Busca um documento específico",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do documento",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeDocumentoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Atualiza os dados de um documento existente",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeDocumentos"
                ],
                "summary": "Atualiza um documento",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do documento",
                        "name": "item",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados atualizados",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeDocumentoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeDocumentoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Realiza a exclusão lógica de um documento",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeDocumentos"
                ],
                "summary": "Exclui um documento",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do documento",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{entidadeId}/documentos/{item}/download": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Download do arquivo de um documento",
                "produces": [
                    "application/octet-stream"
                ],
                "tags": [
                    "EntidadeDocumentos"
                ],
                "summary": "Download do arquivo",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do documento",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Arquivo do documento",
                        "schema": {
                            "type": "file"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{entidadeId}/enderecos": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna todos os endereços de uma entidade",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeEnderecos"
                ],
                "summary": "Lista endereços de uma entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Limite de registros",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Offset para paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por tipo",
                        "name": "tipo",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeEnderecoListResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Cadastra um novo endereço para uma entidade",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeEnderecos"
                ],
                "summary": "Cria um novo endereço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados do endereço",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeEnderecoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeEnderecoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{entidadeId}/enderecos/{item}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna os dados de um endereço específico",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeEnderecos"
                ],
                "summary": "Busca um endereço específico",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do endereço",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeEnderecoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Atualiza os dados de um endereço existente",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeEnderecos"
                ],
                "summary": "Atualiza um endereço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do endereço",
                        "name": "item",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados atualizados",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeEnderecoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeEnderecoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Realiza a exclusão lógica de um endereço",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeEnderecos"
                ],
                "summary": "Exclui um endereço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "entidadeId",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do endereço",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{entidade_id}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna os dados de uma entidade específica",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Entidades"
                ],
                "summary": "Busca entidade por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Atualiza os dados de uma entidade existente",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Entidades"
                ],
                "summary": "Atualiza uma entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados atualizados",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Realiza a exclusão lógica de uma entidade",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Entidades"
                ],
                "summary": "Exclui uma entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da entidade",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{id}/regimes-tributarios": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna todos os regimes tributários de uma entidade",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeRegimesTributarios"
                ],
                "summary": "Lista regimes tributários da entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Entidade",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Limite de registros",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Offset para paginação",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeRegimeTributarioListResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Cadastra um novo regime tributário para uma entidade específica",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeRegimesTributarios"
                ],
                "summary": "Cria um novo regime tributário para a entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Entidade",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados do regime",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeRegimeTributarioRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeRegimeTributarioResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/entidades/{id}/regimes-tributarios/{item}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna os dados de um regime tributário pelo seu item",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeRegimesTributarios"
                ],
                "summary": "Busca um regime tributário específico",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Entidade",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do Item (Regime)",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeRegimeTributarioResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Atualiza os dados de um regime tributário existente",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeRegimesTributarios"
                ],
                "summary": "Atualiza um regime tributário",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Entidade",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do Item (Regime)",
                        "name": "item",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualização",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeRegimeTributarioRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeRegimeTributarioResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Realiza a exclusão lógica de um regime tributário",
                "tags": [
                    "EntidadeRegimesTributarios"
                ],
                "summary": "Exclui um regime tributário",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Entidade",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do Item (Regime)",
                        "name": "item",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/grupos-entidades": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna uma lista paginada de grupos de entidade",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "GrupoEntidades"
                ],
                "summary": "Lista grupos de entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Limite de registros",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Offset para paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoEntidadeListResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Cadastra um novo grupo para ser usado nas entidades",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "GrupoEntidades"
                ],
                "summary": "Cria um novo grupo de entidade",
                "parameters": [
                    {
                        "description": "Dados do grupo",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoEntidadeRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoEntidadeResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/grupos-entidades/{id}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna os dados de um grupo de entidade específico",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "GrupoEntidades"
                ],
                "summary": "Busca grupo por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Grupo",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoEntidadeResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Atualiza o nome de um grupo de entidade",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "GrupoEntidades"
                ],
                "summary": "Atualiza um grupo de entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Grupo",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualização",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoEntidadeRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoEntidadeResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Realiza a exclusão lógica de um grupo de entidade",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "GrupoEntidades"
                ],
                "summary": "Exclui um grupo de entidade",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Grupo",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/grupos-usuario": {
            "get": {
                "description": "Retorna uma lista paginada de grupos de usuário",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Grupo de Usuário"
                ],
                "summary": "Lista todos os grupos de usuário",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Limite de itens por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Número de itens a pular",
                        "name": "offset",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoUsuarioListResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria um novo grupo de usuário com os dados fornecidos",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Grupo de Usuário"
                ],
                "summary": "Cria um novo grupo de usuário",
                "parameters": [
                    {
                        "description": "Dados do Grupo de Usuário",
                        "name": "grupo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoUsuarioRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoUsuarioResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/grupos-usuario/{id}": {
            "get": {
                "description": "Retorna um grupo de usuário específico pelo seu ID",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Grupo de Usuário"
                ],
                "summary": "Busca um grupo de usuário pelo ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Grupo de Usuário",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoUsuarioResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza um grupo de usuário com os dados fornecidos pelo ID",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Grupo de Usuário"
                ],
                "summary": "Atualiza um grupo de usuário existente",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Grupo de Usuário",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados do Grupo de Usuário para atualização",
                        "name": "grupo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoUsuarioRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.GrupoUsuarioResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Exclui logicamente um grupo de usuário pelo ID",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Grupo de Usuário"
                ],
                "summary": "Exclui um grupo de usuário",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Grupo de Usuário",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "409": {
                        "description": "Conflict",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/limites-credito": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna uma lista paginada de limites de crédito",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeLimiteCredito"
                ],
                "summary": "Lista limites de crédito",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Limite de registros",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Offset para paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeLimiteCreditoListResponse"
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "post": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Cadastra um novo limite de crédito para ser usado nas entidades",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeLimiteCredito"
                ],
                "summary": "Cria um novo limite de crédito",
                "parameters": [
                    {
                        "description": "Dados do limite de crédito",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeLimiteCreditoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeLimiteCreditoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "500": {
                        "description": "Internal Server Error",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/limites-credito/{id}": {
            "get": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Retorna os dados de um limite de crédito específico",
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeLimiteCredito"
                ],
                "summary": "Busca limite de crédito por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Limite de Crédito",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeLimiteCreditoResponse"
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "put": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Atualiza a descrição e o valor de um limite de crédito",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "EntidadeLimiteCredito"
                ],
                "summary": "Atualiza um limite de crédito",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Limite de Crédito",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualização",
                        "name": "request",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeLimiteCreditoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.EntidadeLimiteCreditoResponse"
                        }
                    },
                    "400": {
                        "description": "Bad Request",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            },
            "delete": {
                "security": [
                    {
                        "BearerAuth": []
                    }
                ],
                "description": "Realiza a exclusão lógica de um limite de crédito",
                "tags": [
                    "EntidadeLimiteCredito"
                ],
                "summary": "Exclui um limite de crédito",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Limite de Crédito",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "No Content"
                    },
                    "404": {
                        "description": "Not Found",
                        "schema": {
                            "type": "object",
                            "additionalProperties": {
                                "type": "string"
                            }
                        }
                    }
                }
            }
        },
        "/operacoes-fiscais": {
            "get": {
                "description": "Retorna uma lista paginada de operações fiscais, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Operações Fiscais"
                ],
                "summary": "Lista as operações fiscais",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por CFOP",
                        "name": "cfop",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por ID da Empresa Filial",
                        "name": "empresa_filial_id",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.OperacaoFiscalListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria uma nova operação fiscal com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Operações Fiscais"
                ],
                "summary": "Cria uma nova operação fiscal",
                "parameters": [
                    {
                        "description": "Dados para criar a operação fiscal",
                        "name": "operacao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.OperacaoFiscalRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.OperacaoFiscalResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/operacoes-fiscais/cfop/{cfop}": {
            "get": {
                "description": "Retorna operações fiscais filtradas por CFOP.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Operações Fiscais"
                ],
                "summary": "Busca operações fiscais por CFOP",
                "parameters": [
                    {
                        "type": "string",
                        "description": "CFOP da Operação Fiscal",
                        "name": "cfop",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/dto.OperacaoFiscalResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "CFOP inválido",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Operação fiscal não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/operacoes-fiscais/filial/{filial_id}": {
            "get": {
                "description": "Retorna operações fiscais filtradas por ID da filial.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Operações Fiscais"
                ],
                "summary": "Busca operações fiscais por filial",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Filial",
                        "name": "filial_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "type": "array",
                            "items": {
                                "$ref": "#/definitions/dto.OperacaoFiscalResponse"
                            }
                        }
                    },
                    "400": {
                        "description": "ID da filial inválido",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Operação fiscal não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/operacoes-fiscais/{id}": {
            "get": {
                "description": "Retorna os detalhes de uma operação fiscal específica.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Operações Fiscais"
                ],
                "summary": "Busca uma operação fiscal por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Operação Fiscal",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.OperacaoFiscalResponse"
                        }
                    },
                    "404": {
                        "description": "Operação fiscal não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de uma operação fiscal existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Operações Fiscais"
                ],
                "summary": "Atualiza uma operação fiscal",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Operação Fiscal",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar a operação fiscal",
                        "name": "operacao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.OperacaoFiscalRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.OperacaoFiscalResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Operação fiscal não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de uma operação fiscal.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Operações Fiscais"
                ],
                "summary": "Exclui uma operação fiscal",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Operação Fiscal",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/processos": {
            "get": {
                "description": "Retorna uma lista paginada de processos, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Processos"
                ],
                "summary": "Lista os processos",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por código",
                        "name": "codigo",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por tipo de operação (0=Entrada, 1=Saída)",
                        "name": "tipo_operacao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProcessoListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria um novo processo com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Processos"
                ],
                "summary": "Cria um novo processo",
                "parameters": [
                    {
                        "description": "Dados para criar o processo",
                        "name": "processo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProcessoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.ProcessoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/processos/codigo/{codigo}": {
            "get": {
                "description": "Retorna os detalhes de um processo específico pelo código.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Processos"
                ],
                "summary": "Busca um processo por código",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Código do Processo",
                        "name": "codigo",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProcessoResponse"
                        }
                    },
                    "400": {
                        "description": "Código inválido",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Processo não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/processos/{id}": {
            "get": {
                "description": "Retorna os detalhes de um processo específico.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Processos"
                ],
                "summary": "Busca um processo por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Processo",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProcessoResponse"
                        }
                    },
                    "404": {
                        "description": "Processo não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de um processo existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Processos"
                ],
                "summary": "Atualiza um processo",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Processo",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar o processo",
                        "name": "processo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProcessoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProcessoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Processo não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de um processo.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Processos"
                ],
                "summary": "Exclui um processo",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Processo",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produto-variacoes": {
            "get": {
                "description": "Retorna uma lista paginada de variações de produto, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Variações"
                ],
                "summary": "Lista as variações de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "default": 10,
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "default": 0,
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por ID do Produto",
                        "name": "produto_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por ID da Empresa Filial",
                        "name": "empresa_filial_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por ID da Cor",
                        "name": "cor_id",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por ID do Tamanho",
                        "name": "tamanho_id",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por SKU",
                        "name": "sku",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoVariacaoListResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria uma nova variação de produto com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Variações"
                ],
                "summary": "Cria uma nova variação de produto",
                "parameters": [
                    {
                        "description": "Dados para criar a variação de produto",
                        "name": "variacao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoVariacaoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoVariacaoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "409": {
                        "description": "Conflito, por exemplo, SKU já existente",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produto-variacoes/{id}": {
            "get": {
                "description": "Retorna os detalhes de uma variação de produto específica.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Variações"
                ],
                "summary": "Busca uma variação de produto por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Variação de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoVariacaoResponse"
                        }
                    },
                    "400": {
                        "description": "ID da variação de produto inválido",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Variação de produto não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de uma variação de produto existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Variações"
                ],
                "summary": "Atualiza uma variação de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Variação de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar a variação de produto",
                        "name": "variacao",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoVariacaoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoVariacaoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Variação de produto não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "409": {
                        "description": "Conflito, por exemplo, SKU já existente",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de uma variação de produto.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Variações"
                ],
                "summary": "Exclui uma variação de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Variação de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir (ex: variação com estoque)",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Variação de produto não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos": {
            "get": {
                "description": "Retorna uma lista paginada de produtos, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produtos"
                ],
                "summary": "Lista os produtos",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por nome",
                        "name": "nome",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por código",
                        "name": "codigo",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por referência",
                        "name": "referencia",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria um novo produto com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produtos"
                ],
                "summary": "Cria um novo produto",
                "parameters": [
                    {
                        "description": "Dados para criar o produto",
                        "name": "produto",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/cores": {
            "get": {
                "description": "Retorna uma lista paginada de cores de produto, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Cores"
                ],
                "summary": "Lista as cores de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoCorListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria uma nova cor de produto com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Cores"
                ],
                "summary": "Cria uma nova cor de produto",
                "parameters": [
                    {
                        "description": "Dados para criar a cor de produto",
                        "name": "cor",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoCorRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoCorResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/cores/{id}": {
            "get": {
                "description": "Retorna os detalhes de uma cor de produto específica.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Cores"
                ],
                "summary": "Busca uma cor de produto por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Cor de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoCorResponse"
                        }
                    },
                    "404": {
                        "description": "Cor de produto não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de uma cor de produto existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Cores"
                ],
                "summary": "Atualiza uma cor de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Cor de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar a cor de produto",
                        "name": "cor",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoCorRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoCorResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Cor de produto não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de uma cor de produto.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Cores"
                ],
                "summary": "Exclui uma cor de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Cor de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/grupos": {
            "get": {
                "description": "Retorna uma lista paginada de grupos de produto, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Grupos"
                ],
                "summary": "Lista os grupos de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoGrupoListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria um novo grupo de produto com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Grupos"
                ],
                "summary": "Cria um novo grupo de produto",
                "parameters": [
                    {
                        "description": "Dados para criar o grupo de produto",
                        "name": "grupo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoGrupoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoGrupoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/grupos/{id}": {
            "get": {
                "description": "Retorna os detalhes de um grupo de produto específico.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Grupos"
                ],
                "summary": "Busca um grupo de produto por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Grupo de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoGrupoResponse"
                        }
                    },
                    "404": {
                        "description": "Grupo de produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de um grupo de produto existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Grupos"
                ],
                "summary": "Atualiza um grupo de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Grupo de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar o grupo de produto",
                        "name": "grupo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoGrupoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoGrupoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Grupo de produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de um grupo de produto.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Grupos"
                ],
                "summary": "Exclui um grupo de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Grupo de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/marcas": {
            "get": {
                "description": "Retorna uma lista paginada de marcas de produto, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Marcas"
                ],
                "summary": "Lista as marcas de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoMarcaListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria uma nova marca de produto com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Marcas"
                ],
                "summary": "Cria uma nova marca de produto",
                "parameters": [
                    {
                        "description": "Dados para criar a marca de produto",
                        "name": "marca",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoMarcaRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoMarcaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/marcas/{id}": {
            "get": {
                "description": "Retorna os detalhes de uma marca de produto específica.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Marcas"
                ],
                "summary": "Busca uma marca de produto por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Marca de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoMarcaResponse"
                        }
                    },
                    "404": {
                        "description": "Marca de produto não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de uma marca de produto existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Marcas"
                ],
                "summary": "Atualiza uma marca de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Marca de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar a marca de produto",
                        "name": "marca",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoMarcaRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoMarcaResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Marca de produto não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de uma marca de produto.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Marcas"
                ],
                "summary": "Exclui uma marca de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Marca de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/modelos": {
            "get": {
                "description": "Retorna uma lista paginada de modelos de produto, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Modelos"
                ],
                "summary": "Lista os modelos de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoModeloListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria um novo modelo de produto com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Modelos"
                ],
                "summary": "Cria um novo modelo de produto",
                "parameters": [
                    {
                        "description": "Dados para criar o modelo de produto",
                        "name": "modelo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoModeloRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoModeloResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/modelos/{id}": {
            "get": {
                "description": "Retorna os detalhes de um modelo de produto específico.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Modelos"
                ],
                "summary": "Busca um modelo de produto por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Modelo de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoModeloResponse"
                        }
                    },
                    "404": {
                        "description": "Modelo de produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de um modelo de produto existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Modelos"
                ],
                "summary": "Atualiza um modelo de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Modelo de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar o modelo de produto",
                        "name": "modelo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoModeloRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoModeloResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Modelo de produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de um modelo de produto.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Modelos"
                ],
                "summary": "Exclui um modelo de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Modelo de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/subgrupos": {
            "get": {
                "description": "Retorna uma lista paginada de subgrupos de produto, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Subgrupos"
                ],
                "summary": "Lista os subgrupos de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoSubgrupoListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria um novo subgrupo de produto com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Subgrupos"
                ],
                "summary": "Cria um novo subgrupo de produto",
                "parameters": [
                    {
                        "description": "Dados para criar o subgrupo de produto",
                        "name": "subgrupo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoSubgrupoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoSubgrupoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/subgrupos/{id}": {
            "get": {
                "description": "Retorna os detalhes de um subgrupo de produto específico.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Subgrupos"
                ],
                "summary": "Busca um subgrupo de produto por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Subgrupo de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoSubgrupoResponse"
                        }
                    },
                    "404": {
                        "description": "Subgrupo de produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de um subgrupo de produto existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Subgrupos"
                ],
                "summary": "Atualiza um subgrupo de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Subgrupo de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar o subgrupo de produto",
                        "name": "subgrupo",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoSubgrupoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoSubgrupoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Subgrupo de produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de um subgrupo de produto.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Subgrupos"
                ],
                "summary": "Exclui um subgrupo de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Subgrupo de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/tamanhos": {
            "get": {
                "description": "Retorna uma lista paginada de tamanhos de produto, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tamanhos"
                ],
                "summary": "Lista os tamanhos de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoTamanhoListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria um novo tamanho de produto com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tamanhos"
                ],
                "summary": "Cria um novo tamanho de produto",
                "parameters": [
                    {
                        "description": "Dados para criar o tamanho de produto",
                        "name": "tamanho",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoTamanhoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoTamanhoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/tamanhos/{id}": {
            "get": {
                "description": "Retorna os detalhes de um tamanho de produto específico.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tamanhos"
                ],
                "summary": "Busca um tamanho de produto por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Tamanho de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoTamanhoResponse"
                        }
                    },
                    "404": {
                        "description": "Tamanho de produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de um tamanho de produto existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tamanhos"
                ],
                "summary": "Atualiza um tamanho de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Tamanho de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar o tamanho de produto",
                        "name": "tamanho",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoTamanhoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoTamanhoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Tamanho de produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de um tamanho de produto.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tamanhos"
                ],
                "summary": "Exclui um tamanho de produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Tamanho de Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/produtos/{id}": {
            "get": {
                "description": "Retorna os detalhes de um produto específico.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produtos"
                ],
                "summary": "Busca um produto por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoResponse"
                        }
                    },
                    "404": {
                        "description": "Produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de um produto existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produtos"
                ],
                "summary": "Atualiza um produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar o produto",
                        "name": "produto",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.ProdutoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Produto não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de um produto.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produtos"
                ],
                "summary": "Exclui um produto",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID do Produto",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/tabelas-preco": {
            "get": {
                "description": "Retorna uma lista paginada de tabelas de preço, com suporte a filtros.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Lista as tabelas de preço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por descrição",
                        "name": "descricao",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Filtrar por situação (1=Ativo, 2=Inativo)",
                        "name": "situacao",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Cria uma nova tabela de preço com base nos dados fornecidos.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Cria uma nova tabela de preço",
                "parameters": [
                    {
                        "description": "Dados para criar a tabela de preço",
                        "name": "tabela",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "500": {
                        "description": "Erro interno do servidor",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/tabelas-preco/{tabela_id}": {
            "get": {
                "description": "Retorna os detalhes de uma tabela de preço específica.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Busca uma tabela de preço por ID",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Tabela de Preço",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoResponse"
                        }
                    },
                    "404": {
                        "description": "Tabela de preço não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de uma tabela de preço existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Atualiza uma tabela de preço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Tabela de Preço",
                        "name": "id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar a tabela de preço",
                        "name": "tabela",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Tabela de preço não encontrada",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de uma tabela de preço.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Exclui uma tabela de preço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Tabela de Preço",
                        "name": "id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/tabelas-preco/{tabela_id}/produtos": {
            "get": {
                "description": "Retorna uma lista paginada de produtos de uma tabela de preço.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Lista os produtos de uma tabela de preço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Tabela de Preço",
                        "name": "tabela_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "Número de registros por página",
                        "name": "limit",
                        "in": "query"
                    },
                    {
                        "type": "integer",
                        "description": "Offset para a paginação",
                        "name": "offset",
                        "in": "query"
                    },
                    {
                        "type": "string",
                        "description": "Filtrar por nome do produto",
                        "name": "produto_nome",
                        "in": "query"
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoProdutoListResponse"
                        }
                    }
                }
            },
            "post": {
                "description": "Adiciona um novo produto com seus valores a uma tabela de preço existente.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Adiciona um produto a uma tabela de preço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Tabela de Preço",
                        "name": "tabela_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados do produto na tabela",
                        "name": "produto",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoProdutoRequest"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created",
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoProdutoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação ou dados inválidos",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        },
        "/tabelas-preco/{tabela_id}/produtos/{item_id}": {
            "get": {
                "description": "Retorna os detalhes de um produto específico dentro de uma tabela de preço.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Busca um produto em uma tabela de preço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Tabela de Preço",
                        "name": "tabela_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do Item (Produto na Tabela)",
                        "name": "item_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoProdutoResponse"
                        }
                    },
                    "404": {
                        "description": "Item não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "put": {
                "description": "Atualiza os dados de um produto em uma tabela de preço.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Atualiza um produto em uma tabela de preço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Tabela de Preço",
                        "name": "tabela_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do Item (Produto na Tabela)",
                        "name": "item_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "description": "Dados para atualizar",
                        "name": "produto",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoProdutoRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "OK",
                        "schema": {
                            "$ref": "#/definitions/dto.TabelaPrecoProdutoResponse"
                        }
                    },
                    "400": {
                        "description": "Erro de validação",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    },
                    "404": {
                        "description": "Item não encontrado",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            },
            "delete": {
                "description": "Realiza a exclusão lógica de um produto de uma tabela de preço.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                    "application/json"
                ],
                "tags": [
                    "Produto - Tabela de Preços"
                ],
                "summary": "Remove um produto de uma tabela de preço",
                "parameters": [
                    {
                        "type": "integer",
                        "description": "ID da Tabela de Preço",
                        "name": "tabela_id",
                        "in": "path",
                        "required": true
                    },
                    {
                        "type": "integer",
                        "description": "ID do Item (Produto na Tabela)",
                        "name": "item_id",
                        "in": "path",
                        "required": true
                    }
                ],
                "responses": {
                    "204": {
                        "description": "Nenhum conteúdo"
                    },
                    "400": {
                        "description": "Erro ao excluir",
                        "schema": {
                            "$ref": "#/definitions/utils.ErrorResponse"
                        }
                    }
                }
            }
        }
    },
    "definitions": {
        "constants.EstadoCivil": {
            "type": "integer",
            "enum": [
                1,
                2,
                3,
                4,
                5,
                6
            ],
            "x-enum-varnames": [
                "EstadoCivilSolteiro",
                "EstadoCivilCasado",
                "EstadoCivilDivorciado",
                "EstadoCivilViuvo",
                "EstadoCivilSeparado",
                "EstadoCivilUniaoEstavel"
            ]
        },
        "constants.Sexo": {
            "type": "integer",
            "enum": [
                1,
                2
            ],
            "x-enum-varnames": [
                "SexoMasculino",
                "SexoFeminino"
            ]
        },
        "constants.Status": {
            "type": "integer",
            "enum": [
                1,
                2,
                3,
                9
            ],
            "x-enum-varnames": [
                "StatusAtivo",
                "StatusInativo",
                "StatusBloqueado",
                "StatusCancelado"
            ]
        },
        "constants.TipoPessoa": {
            "type": "integer",
            "enum": [
                1,
                2
            ],
            "x-enum-varnames": [
                "TipoPessoaFisica",
                "TipoPessoaJuridica"
            ]
        },
        "dto.CondicaoPagamentoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.CondicaoPagamentoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.CondicaoPagamentoRequest": {
            "type": "object",
            "required": [
                "descricao",
                "portador_id",
                "tipo_condicao",
                "tipo_documento_id"
            ],
            "properties": {
                "comissao": {
                    "type": "number"
                },
                "created_by": {
                    "description": "============================================================\nUSUÁRIO (para auditoria)\n============================================================",
                    "type": "integer"
                },
                "desconto": {
                    "type": "number"
                },
                "descricao": {
                    "type": "string"
                },
                "dia_pagamento": {
                    "type": "integer"
                },
                "dias_parcelas": {
                    "type": "integer"
                },
                "entrada": {
                    "type": "integer"
                },
                "forma_pagamento_id": {
                    "type": "integer"
                },
                "juros": {
                    "type": "number"
                },
                "numero_parcelas": {
                    "type": "integer"
                },
                "portador_id": {
                    "type": "integer"
                },
                "rotina_contabil_id": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "situacao": {
                    "type": "integer"
                },
                "tipo_condicao": {
                    "type": "integer"
                },
                "tipo_documento_id": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.CondicaoPagamentoResponse": {
            "type": "object",
            "properties": {
                "comissao": {
                    "type": "number"
                },
                "created_at": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "desconto": {
                    "type": "number"
                },
                "descricao": {
                    "type": "string"
                },
                "dia_pagamento": {
                    "type": "integer"
                },
                "dias_parcelas": {
                    "type": "integer"
                },
                "entrada": {
                    "type": "integer"
                },
                "entrada_label": {
                    "type": "string"
                },
                "forma_pagamento_id": {
                    "type": "integer"
                },
                "forma_pagamento_nome": {
                    "type": "string"
                },
                "id": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "juros": {
                    "type": "number"
                },
                "numero_parcelas": {
                    "type": "integer"
                },
                "portador_id": {
                    "type": "integer"
                },
                "portador_nome": {
                    "type": "string"
                },
                "rotina_contabil_id": {
                    "type": "integer"
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "tipo_condicao": {
                    "type": "integer"
                },
                "tipo_condicao_label": {
                    "type": "string"
                },
                "tipo_documento_id": {
                    "type": "integer"
                },
                "tipo_documento_nome": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.DocumentoVendaItemRequest": {
            "type": "object",
            "required": [
                "produto_id",
                "quantidade"
            ],
            "properties": {
                "created_by": {
                    "type": "integer"
                },
                "cst_icms_id": {
                    "type": "integer"
                },
                "cst_ipi_id": {
                    "type": "integer"
                },
                "cst_pis_cofins_id": {
                    "type": "integer"
                },
                "descricao_produto": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "operacao_fiscal_id": {
                    "type": "integer"
                },
                "percentual_desconto": {
                    "type": "number"
                },
                "peso_bruto": {
                    "type": "number"
                },
                "peso_liquido": {
                    "type": "number"
                },
                "produto_id": {
                    "type": "integer"
                },
                "quantidade": {
                    "type": "number"
                },
                "updated_by": {
                    "type": "integer"
                },
                "valor_desconto": {
                    "type": "number"
                },
                "valor_frete": {
                    "type": "number"
                },
                "valor_unitario": {
                    "type": "number",
                    "minimum": 0
                }
            }
        },
        "dto.DocumentoVendaItemResponse": {
            "type": "object",
            "properties": {
                "documento_venda_id": {
                    "type": "integer"
                },
                "item": {
                    "type": "integer"
                },
                "percentual_desconto": {
                    "type": "number"
                },
                "peso_bruto": {
                    "type": "number"
                },
                "peso_liquido": {
                    "type": "number"
                },
                "produto_codigo": {
                    "type": "integer"
                },
                "produto_id": {
                    "type": "integer"
                },
                "produto_nome": {
                    "type": "string"
                },
                "quantidade": {
                    "type": "number"
                },
                "total_item": {
                    "type": "number"
                },
                "total_item_com_frete": {
                    "type": "number"
                },
                "valor_desconto": {
                    "type": "number"
                },
                "valor_frete": {
                    "type": "number"
                },
                "valor_unitario": {
                    "type": "number"
                }
            }
        },
        "dto.DocumentoVendaListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.DocumentoVendaResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.DocumentoVendaPagamentoRequest": {
            "type": "object",
            "required": [
                "data_vencimento",
                "portador_id",
                "tipo_documento_id",
                "valor"
            ],
            "properties": {
                "autorizacao": {
                    "type": "string"
                },
                "bandeira_cartao": {
                    "type": "string"
                },
                "codigo_autorizacao": {
                    "type": "string"
                },
                "coo": {
                    "type": "integer"
                },
                "created_by": {
                    "type": "integer"
                },
                "data_vencimento": {
                    "description": "Changed to *string for parsing, and required",
                    "type": "string"
                },
                "documento": {
                    "type": "string"
                },
                "documento_venda_id": {
                    "description": "Changed from \"id\"",
                    "type": "integer"
                },
                "forma_pagamento_id": {
                    "description": "Changed to *int",
                    "type": "integer"
                },
                "nsu": {
                    "type": "string"
                },
                "portador_id": {
                    "type": "integer"
                },
                "rotina_contabil_id": {
                    "type": "integer"
                },
                "tipo_documento_id": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                },
                "valor": {
                    "type": "number"
                }
            }
        },
        "dto.DocumentoVendaPagamentoResponse": {
            "type": "object",
            "properties": {
                "autorizacao": {
                    "type": "string"
                },
                "bandeira_cartao": {
                    "type": "string"
                },
                "codigo_autorizacao": {
                    "type": "string"
                },
                "coo": {
                    "type": "integer"
                },
                "created_at": {
                    "description": "Formatted string",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "data_vencimento": {
                    "type": "string"
                },
                "documento": {
                    "type": "string"
                },
                "documento_venda_id": {
                    "type": "integer"
                },
                "forma_pagamento_id": {
                    "type": "integer"
                },
                "forma_pagamento_nome": {
                    "type": "string"
                },
                "item": {
                    "description": "Sequencial por documento",
                    "type": "integer"
                },
                "nsu": {
                    "type": "string"
                },
                "portador_id": {
                    "type": "integer"
                },
                "portador_nome": {
                    "type": "string"
                },
                "rotina_contabil_id": {
                    "type": "integer"
                },
                "status": {
                    "description": "Added from model",
                    "type": "integer"
                },
                "status_label": {
                    "description": "Added for display",
                    "type": "string"
                },
                "tipo_documento_id": {
                    "type": "integer"
                },
                "tipo_documento_nome": {
                    "type": "string"
                },
                "updated_at": {
                    "description": "Formatted string",
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                },
                "valor": {
                    "type": "number"
                }
            }
        },
        "dto.DocumentoVendaRequest": {
            "type": "object",
            "required": [
                "condicao_pagamento_id",
                "empresa_filial_id",
                "tabela_preco_id",
                "tipo_documento",
                "tipo_operacao"
            ],
            "properties": {
                "condicao_pagamento_id": {
                    "type": "integer"
                },
                "created_by": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "integer"
                },
                "data_previsao": {
                    "type": "string"
                },
                "data_validade": {
                    "description": "============================================================\nDATAS\n============================================================",
                    "type": "string"
                },
                "empresa_filial_id": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "entidade_id": {
                    "type": "integer"
                },
                "itens": {
                    "description": "============================================================\nLISTAS\n============================================================",
                    "type": "array",
                    "minItems": 1,
                    "items": {
                        "$ref": "#/definitions/dto.DocumentoVendaItemRequest"
                    }
                },
                "numero": {
                    "type": "integer"
                },
                "observacoes_adm": {
                    "type": "string"
                },
                "observacoes_cliente": {
                    "type": "string"
                },
                "observacoes_interna": {
                    "description": "============================================================\nOBSERVAÇÕES\n============================================================",
                    "type": "string"
                },
                "pagamentos": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.DocumentoVendaPagamentoRequest"
                    }
                },
                "tabela_preco_id": {
                    "type": "integer"
                },
                "tipo_documento": {
                    "description": "1-Orçamento, 2-Pedido",
                    "type": "integer"
                },
                "tipo_entrega": {
                    "description": "RETIRADA, ENTREGA, LOCAL",
                    "type": "string"
                },
                "tipo_operacao": {
                    "description": "0-Entrada, 1-Saída",
                    "type": "integer"
                },
                "transportadora_id": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                },
                "valor_desconto": {
                    "type": "number"
                },
                "valor_frete": {
                    "description": "============================================================\nVALORES\n============================================================",
                    "type": "number"
                },
                "vendedor_id": {
                    "description": "============================================================\nRELACIONAMENTOS\n============================================================",
                    "type": "integer"
                }
            }
        },
        "dto.DocumentoVendaResponse": {
            "type": "object",
            "properties": {
                "condicao_pagamento_descricao": {
                    "type": "string"
                },
                "condicao_pagamento_id": {
                    "type": "integer"
                },
                "created_at": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "data_documento": {
                    "description": "============================================================\nDATAS\n============================================================",
                    "type": "string"
                },
                "data_previsao": {
                    "type": "string"
                },
                "data_validade": {
                    "type": "string"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "entidade_documento": {
                    "type": "string"
                },
                "entidade_id": {
                    "type": "integer"
                },
                "entidade_nome": {
                    "type": "string"
                },
                "id": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "itens": {
                    "description": "============================================================\nLISTAS\n============================================================",
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.DocumentoVendaItemResponse"
                    }
                },
                "numero": {
                    "type": "integer"
                },
                "observacoes_adm": {
                    "type": "string"
                },
                "observacoes_cliente": {
                    "type": "string"
                },
                "observacoes_interna": {
                    "description": "============================================================\nOBSERVAÇÕES\n============================================================",
                    "type": "string"
                },
                "pagamentos": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.DocumentoVendaPagamentoResponse"
                    }
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "tabela_preco_descricao": {
                    "type": "string"
                },
                "tabela_preco_id": {
                    "type": "integer"
                },
                "tipo_documento": {
                    "type": "integer"
                },
                "tipo_documento_label": {
                    "type": "string"
                },
                "tipo_entrega": {
                    "type": "string"
                },
                "tipo_operacao": {
                    "type": "integer"
                },
                "tipo_operacao_label": {
                    "type": "string"
                },
                "total_descontos": {
                    "type": "number"
                },
                "total_frete": {
                    "type": "number"
                },
                "total_liquido": {
                    "type": "number"
                },
                "total_produtos": {
                    "description": "============================================================\nVALORES\n============================================================",
                    "type": "number"
                },
                "transportadora_id": {
                    "type": "integer"
                },
                "transportadora_nome": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                },
                "valor_desconto": {
                    "type": "number"
                },
                "valor_frete": {
                    "type": "number"
                },
                "vendedor_id": {
                    "description": "============================================================\nRELACIONAMENTOS\n============================================================",
                    "type": "integer"
                },
                "vendedor_nome": {
                    "type": "string"
                }
            }
        },
        "dto.EmpresaFilialListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.EmpresaFilialResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.EmpresaFilialRequest": {
            "type": "object",
            "required": [
                "aliquota_cofins",
                "aliquota_pis",
                "empresa_id",
                "nome",
                "numero"
            ],
            "properties": {
                "aliquota_cofins": {
                    "type": "number",
                    "maximum": 99.99,
                    "minimum": 0
                },
                "aliquota_funrural": {
                    "type": "number",
                    "maximum": 99.99,
                    "minimum": 0
                },
                "aliquota_iss": {
                    "type": "number",
                    "maximum": 99.99,
                    "minimum": 0
                },
                "aliquota_pis": {
                    "type": "number",
                    "maximum": 99.99,
                    "minimum": 0
                },
                "codigo_cnae": {
                    "type": "string",
                    "maxLength": 20
                },
                "created_by": {
                    "type": "integer"
                },
                "empresa_id": {
                    "type": "integer"
                },
                "endereco_web": {
                    "type": "string",
                    "maxLength": 1000
                },
                "entidade_id": {
                    "type": "integer"
                },
                "logomarca_web": {
                    "type": "string",
                    "maxLength": 1000
                },
                "mei": {
                    "description": "0 ou 1",
                    "type": "integer"
                },
                "nome": {
                    "type": "string",
                    "maxLength": 100
                },
                "numero": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EmpresaFilialResponse": {
            "type": "object",
            "properties": {
                "aliquota_cofins": {
                    "type": "number"
                },
                "aliquota_funrural": {
                    "type": "number"
                },
                "aliquota_iss": {
                    "type": "number"
                },
                "aliquota_pis": {
                    "type": "number"
                },
                "codigo_cnae": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "empresa_id": {
                    "type": "integer"
                },
                "empresa_nome": {
                    "type": "string"
                },
                "endereco_web": {
                    "type": "string"
                },
                "entidade_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "is_deleted": {
                    "type": "boolean"
                },
                "logomarca_web": {
                    "type": "string"
                },
                "mei": {
                    "type": "integer"
                },
                "mei_label": {
                    "type": "string"
                },
                "nome": {
                    "type": "string"
                },
                "numero": {
                    "type": "integer"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeContatoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.EntidadeContatoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeContatoRequest": {
            "type": "object",
            "required": [
                "entidade_id",
                "forma_contato_id",
                "informacao"
            ],
            "properties": {
                "created_by": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "entidade_id": {
                    "description": "============================================================\nCAMPOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "forma_contato_id": {
                    "type": "integer",
                    "maximum": 7,
                    "minimum": 1
                },
                "informacao": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeContatoResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "entidade_id": {
                    "description": "============================================================\nCAMPOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "forma_contato_id": {
                    "type": "integer"
                },
                "forma_contato_nome": {
                    "description": "Nome da forma de contato",
                    "type": "string"
                },
                "forma_contato_tipo": {
                    "description": "Tipo da forma de contato",
                    "type": "integer"
                },
                "forma_contato_tipo_label": {
                    "type": "string"
                },
                "informacao": {
                    "type": "string"
                },
                "item": {
                    "description": "Sequencial por entidade",
                    "type": "integer"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeDocumentoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.EntidadeDocumentoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeDocumentoRequest": {
            "type": "object",
            "required": [
                "arquivo",
                "data_inclusao",
                "entidade_id"
            ],
            "properties": {
                "arquivo": {
                    "description": "Base64",
                    "type": "string"
                },
                "created_by": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "integer"
                },
                "data_inclusao": {
                    "description": "Formato: 2006-01-02",
                    "type": "string"
                },
                "descricao": {
                    "type": "string"
                },
                "entidade_id": {
                    "description": "============================================================\nCAMPOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "tipo": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeDocumentoResponse": {
            "type": "object",
            "properties": {
                "arquivo": {
                    "description": "Base64 (opcional)",
                    "type": "string"
                },
                "content_type": {
                    "description": "Tipo do arquivo (ex: application/pdf)",
                    "type": "string"
                },
                "created_at": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "data_inclusao": {
                    "type": "string"
                },
                "descricao": {
                    "type": "string"
                },
                "entidade_id": {
                    "description": "============================================================\nCAMPOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "item": {
                    "description": "Sequencial por entidade",
                    "type": "integer"
                },
                "tamanho": {
                    "description": "============================================================\nMETADADOS\n============================================================",
                    "type": "integer"
                },
                "tipo": {
                    "type": "string"
                },
                "tipo_label": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeEnderecoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.EntidadeEnderecoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeEnderecoRequest": {
            "type": "object",
            "required": [
                "bairro",
                "cep",
                "data_ini",
                "entidade_id",
                "estado_id",
                "logradouro",
                "municipio_id",
                "numero",
                "pais_id",
                "tipo"
            ],
            "properties": {
                "bairro": {
                    "type": "string"
                },
                "cep": {
                    "type": "string"
                },
                "complemento": {
                    "type": "string"
                },
                "created_by": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "integer"
                },
                "data_fim": {
                    "type": "string"
                },
                "data_ini": {
                    "description": "============================================================\nDATAS\n============================================================",
                    "type": "string"
                },
                "distancia": {
                    "type": "number"
                },
                "entidade_id": {
                    "description": "============================================================\nCAMPOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "estado_id": {
                    "type": "integer"
                },
                "logradouro": {
                    "type": "string"
                },
                "municipio_id": {
                    "type": "integer"
                },
                "numero": {
                    "type": "string"
                },
                "observacao": {
                    "type": "string"
                },
                "pais_id": {
                    "type": "integer"
                },
                "situacao": {
                    "description": "1-Ativo, 2-Inativo",
                    "type": "integer"
                },
                "tipo": {
                    "type": "integer",
                    "maximum": 4,
                    "minimum": 1
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeEnderecoResponse": {
            "type": "object",
            "properties": {
                "bairro": {
                    "type": "string"
                },
                "cep": {
                    "type": "string"
                },
                "complemento": {
                    "type": "string"
                },
                "created_at": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "data_fim": {
                    "type": "string"
                },
                "data_ini": {
                    "description": "============================================================\nDATAS\n============================================================",
                    "type": "string"
                },
                "distancia": {
                    "type": "number"
                },
                "entidade_id": {
                    "description": "============================================================\nCAMPOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "estado_id": {
                    "type": "integer"
                },
                "estado_nome": {
                    "type": "string"
                },
                "estado_uf": {
                    "type": "string"
                },
                "item": {
                    "description": "Sequencial por entidade",
                    "type": "integer"
                },
                "logradouro": {
                    "type": "string"
                },
                "municipio_id": {
                    "type": "integer"
                },
                "municipio_nome": {
                    "type": "string"
                },
                "numero": {
                    "type": "string"
                },
                "observacao": {
                    "type": "string"
                },
                "pais_id": {
                    "type": "integer"
                },
                "pais_nome": {
                    "type": "string"
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "tipo": {
                    "type": "integer"
                },
                "tipo_label": {
                    "description": "Cobrança, Entrega, Comercial, Residencial",
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeLimiteCreditoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.EntidadeLimiteCreditoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeLimiteCreditoRequest": {
            "type": "object",
            "required": [
                "descricao",
                "valor"
            ],
            "properties": {
                "descricao": {
                    "type": "string",
                    "maxLength": 100
                },
                "valor": {
                    "type": "number"
                }
            }
        },
        "dto.EntidadeLimiteCreditoResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "descricao": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "updated_at": {
                    "type": "string"
                },
                "valor": {
                    "type": "number"
                }
            }
        },
        "dto.EntidadeListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.EntidadeResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeRegimeTributarioListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.EntidadeRegimeTributarioResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeRegimeTributarioRequest": {
            "type": "object",
            "required": [
                "data",
                "entidade_id",
                "regime"
            ],
            "properties": {
                "apuracao": {
                    "type": "integer"
                },
                "created_by": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "integer"
                },
                "data": {
                    "description": "Formato: 2006-01-02",
                    "type": "string"
                },
                "entidade_id": {
                    "description": "============================================================\nCAMPOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "regime": {
                    "type": "integer"
                },
                "regime_especial": {
                    "type": "integer"
                },
                "regime_municipal": {
                    "type": "integer"
                },
                "situacao_trib_iss": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeRegimeTributarioResponse": {
            "type": "object",
            "properties": {
                "apuracao": {
                    "type": "integer"
                },
                "apuracao_label": {
                    "type": "string"
                },
                "created_at": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "data": {
                    "type": "string"
                },
                "entidade_id": {
                    "type": "integer"
                },
                "id": {
                    "description": "============================================================\nCAMPOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "regime": {
                    "type": "integer"
                },
                "regime_especial": {
                    "type": "integer"
                },
                "regime_especial_label": {
                    "type": "string"
                },
                "regime_label": {
                    "type": "string"
                },
                "regime_municipal": {
                    "type": "integer"
                },
                "regime_municipal_label": {
                    "type": "string"
                },
                "situacao_trib_iss": {
                    "type": "integer"
                },
                "situacao_trib_iss_label": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeRequest": {
            "type": "object",
            "required": [
                "empresa_filial_id",
                "inscricao_federal",
                "razao_social",
                "tipo_pessoa"
            ],
            "properties": {
                "arquivo_imp_ddv": {
                    "type": "string"
                },
                "casa_propria": {
                    "type": "integer"
                },
                "classificacao": {
                    "type": "integer"
                },
                "codigo": {
                    "type": "integer"
                },
                "conjuje_cpf": {
                    "type": "string"
                },
                "conjuje_nome": {
                    "type": "string"
                },
                "conjuje_renda": {
                    "type": "number"
                },
                "conjuje_rg": {
                    "type": "string"
                },
                "created_by": {
                    "description": "============================================================\nUSUÁRIO (para auditoria)\n============================================================",
                    "type": "integer"
                },
                "data_cadastro": {
                    "description": "============================================================\nDADOS COMERCIAIS\n============================================================",
                    "type": "string"
                },
                "data_nascimento": {
                    "description": "============================================================\nDADOS PESSOAIS\n============================================================",
                    "type": "string"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "estado_civil": {
                    "type": "integer"
                },
                "grupo_entidade_id": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "horario_id": {
                    "type": "integer"
                },
                "inscricao_estadual": {
                    "type": "string"
                },
                "inscricao_federal": {
                    "type": "string"
                },
                "inscricao_municipal": {
                    "type": "string"
                },
                "inscricao_produtor": {
                    "type": "string"
                },
                "nome_da_mae": {
                    "type": "string"
                },
                "nome_do_pai": {
                    "type": "string"
                },
                "nome_fantasia": {
                    "type": "string"
                },
                "observacao": {
                    "type": "string"
                },
                "percentual_comissao": {
                    "type": "number"
                },
                "quant_filhos": {
                    "type": "integer"
                },
                "razao_social": {
                    "type": "string"
                },
                "sexo": {
                    "$ref": "#/definitions/constants.Sexo"
                },
                "situacao": {
                    "$ref": "#/definitions/constants.Status"
                },
                "suframa": {
                    "type": "string"
                },
                "tabela_desconto_id": {
                    "type": "integer"
                },
                "tabela_preco_id": {
                    "type": "integer"
                },
                "taxa_entrega": {
                    "type": "number"
                },
                "tipo_pessoa": {
                    "maximum": 2,
                    "minimum": 1,
                    "allOf": [
                        {
                            "$ref": "#/definitions/constants.TipoPessoa"
                        }
                    ]
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.EntidadeResponse": {
            "type": "object",
            "properties": {
                "arquivo_imp_ddv": {
                    "type": "string"
                },
                "casa_propria": {
                    "type": "integer"
                },
                "classificacao": {
                    "type": "integer"
                },
                "codigo": {
                    "type": "integer"
                },
                "conjuje_cpf": {
                    "type": "string"
                },
                "conjuje_nome": {
                    "type": "string"
                },
                "conjuje_renda": {
                    "type": "number"
                },
                "conjuje_rg": {
                    "type": "string"
                },
                "created_at": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "data_cadastro": {
                    "description": "============================================================\nDADOS COMERCIAIS\n============================================================",
                    "type": "string"
                },
                "data_nascimento": {
                    "description": "============================================================\nDADOS PESSOAIS\n============================================================",
                    "type": "string"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "estado_civil": {
                    "$ref": "#/definitions/constants.EstadoCivil"
                },
                "estado_civil_label": {
                    "type": "string"
                },
                "grupo_entidade_id": {
                    "type": "integer"
                },
                "horario_id": {
                    "type": "integer"
                },
                "id": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "inscricao_estadual": {
                    "type": "string"
                },
                "inscricao_federal": {
                    "type": "string"
                },
                "inscricao_municipal": {
                    "type": "string"
                },
                "inscricao_produtor": {
                    "type": "string"
                },
                "nome_da_mae": {
                    "type": "string"
                },
                "nome_do_pai": {
                    "type": "string"
                },
                "nome_fantasia": {
                    "type": "string"
                },
                "observacao": {
                    "type": "string"
                },
                "percentual_comissao": {
                    "type": "number"
                },
                "quant_filhos": {
                    "type": "integer"
                },
                "razao_social": {
                    "type": "string"
                },
                "sexo": {
                    "$ref": "#/definitions/constants.Sexo"
                },
                "sexo_label": {
                    "type": "string"
                },
                "situacao": {
                    "$ref": "#/definitions/constants.Status"
                },
                "situacao_label": {
                    "type": "string"
                },
                "suframa": {
                    "type": "string"
                },
                "tabela_desconto_id": {
                    "type": "integer"
                },
                "tabela_preco_id": {
                    "type": "integer"
                },
                "taxa_entrega": {
                    "type": "number"
                },
                "tipo_pessoa": {
                    "$ref": "#/definitions/constants.TipoPessoa"
                },
                "tipo_pessoa_label": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ErrorResponse": {
            "type": "object",
            "properties": {
                "code": {
                    "type": "integer"
                },
                "error": {
                    "type": "string"
                },
                "message": {
                    "type": "string"
                }
            }
        },
        "dto.GrupoEntidadeListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.GrupoEntidadeResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.GrupoEntidadeRequest": {
            "type": "object",
            "required": [
                "descricao"
            ],
            "properties": {
                "descricao": {
                    "type": "string",
                    "maxLength": 100
                }
            }
        },
        "dto.GrupoEntidadeResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "descricao": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "updated_at": {
                    "type": "string"
                }
            }
        },
        "dto.GrupoUsuarioListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.GrupoUsuarioResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.GrupoUsuarioRequest": {
            "type": "object",
            "required": [
                "descricao",
                "empresa_filial_id"
            ],
            "properties": {
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string",
                    "maxLength": 100
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "situacao": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.GrupoUsuarioResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "empresa_filial_nome": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "situacao": {
                    "$ref": "#/definitions/constants.Status"
                },
                "situacao_label": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.LoginRequest": {
            "type": "object",
            "required": [
                "login",
                "senha"
            ],
            "properties": {
                "empresa_id": {
                    "description": "Opcional para multi-empresa",
                    "type": "integer"
                },
                "login": {
                    "type": "string"
                },
                "senha": {
                    "type": "string"
                }
            }
        },
        "dto.LoginResponse": {
            "type": "object",
            "properties": {
                "access_token": {
                    "type": "string"
                },
                "expires_in": {
                    "description": "Segundos",
                    "type": "integer"
                },
                "refresh_token": {
                    "type": "string"
                },
                "token_type": {
                    "type": "string"
                },
                "usuario": {
                    "$ref": "#/definitions/dto.UsuarioInfo"
                }
            }
        },
        "dto.OperacaoFiscalListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.OperacaoFiscalResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.OperacaoFiscalRequest": {
            "type": "object",
            "required": [
                "cfop",
                "cst_icms_id",
                "cst_ipi_id",
                "cst_pis_cofins_id",
                "data_ini",
                "descricao",
                "empresa_filial_id"
            ],
            "properties": {
                "cfop": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "cst_icms_id": {
                    "type": "integer"
                },
                "cst_ipi_id": {
                    "type": "integer"
                },
                "cst_pis_cofins_id": {
                    "type": "integer"
                },
                "data_fim": {
                    "description": "Formato: YYYY-MM-DD",
                    "type": "string"
                },
                "data_ini": {
                    "description": "Formato: YYYY-MM-DD",
                    "type": "string"
                },
                "descricao": {
                    "type": "string",
                    "maxLength": 255
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.OperacaoFiscalResponse": {
            "type": "object",
            "properties": {
                "cfop": {
                    "type": "string"
                },
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "cst_icms_codigo": {
                    "type": "string"
                },
                "cst_icms_id": {
                    "type": "integer"
                },
                "cst_ipi_codigo": {
                    "type": "string"
                },
                "cst_ipi_id": {
                    "type": "integer"
                },
                "cst_pis_cofins_codigo": {
                    "type": "string"
                },
                "cst_pis_cofins_id": {
                    "type": "integer"
                },
                "data_fim": {
                    "type": "string"
                },
                "data_ini": {
                    "type": "string"
                },
                "descricao": {
                    "type": "string"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "empresa_filial_nome": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProcessoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.ProcessoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.ProcessoRequest": {
            "type": "object",
            "required": [
                "codigo",
                "descricao",
                "empresa_filial_id",
                "tipo_operacao"
            ],
            "properties": {
                "codigo": {
                    "type": "integer"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string",
                    "maxLength": 255
                },
                "despesa_id": {
                    "type": "integer"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "natureza_operacao_id": {
                    "type": "integer"
                },
                "operacao_fiscal_fora_est_id": {
                    "type": "integer"
                },
                "operacao_fiscal_fora_est_st_id": {
                    "type": "integer"
                },
                "operacao_fiscal_no_est_id": {
                    "type": "integer"
                },
                "operacao_fiscal_no_est_st_id": {
                    "type": "integer"
                },
                "plano_contas_financeiro_id": {
                    "type": "integer"
                },
                "processo_nf_id": {
                    "type": "integer"
                },
                "receita_id": {
                    "type": "integer"
                },
                "rotina_contabil_id": {
                    "type": "integer"
                },
                "situacao": {
                    "type": "integer"
                },
                "tipo_operacao": {
                    "description": "0-Entrada, 1-Saída",
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProcessoResponse": {
            "type": "object",
            "properties": {
                "codigo": {
                    "type": "integer"
                },
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "despesa_id": {
                    "type": "integer"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "empresa_filial_nome": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "natureza_operacao_id": {
                    "type": "integer"
                },
                "operacao_fiscal_fora_est_id": {
                    "type": "integer"
                },
                "operacao_fiscal_fora_est_st_id": {
                    "type": "integer"
                },
                "operacao_fiscal_no_est_id": {
                    "type": "integer"
                },
                "operacao_fiscal_no_est_st_id": {
                    "type": "integer"
                },
                "plano_contas_financeiro_id": {
                    "type": "integer"
                },
                "processo_nf_id": {
                    "type": "integer"
                },
                "receita_id": {
                    "type": "integer"
                },
                "rotina_contabil_descricao": {
                    "type": "string"
                },
                "rotina_contabil_id": {
                    "type": "integer"
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "tipo_operacao": {
                    "type": "integer"
                },
                "tipo_operacao_label": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoCorListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.ProdutoCorResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoCorRequest": {
            "type": "object",
            "required": [
                "descricao",
                "empresa_filial_id"
            ],
            "properties": {
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string",
                    "maxLength": 255
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "sigla": {
                    "type": "string",
                    "maxLength": 10
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoCorResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "empresa_filial_nome": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "sigla": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoGrupoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.ProdutoGrupoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoGrupoRequest": {
            "type": "object",
            "required": [
                "descricao"
            ],
            "properties": {
                "agenda": {
                    "type": "integer"
                },
                "controle_lote": {
                    "type": "integer"
                },
                "cor": {
                    "type": "string"
                },
                "created_by": {
                    "description": "============================================================\nUSUÁRIO (para auditoria)\n============================================================",
                    "type": "integer"
                },
                "descricao": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "string"
                },
                "impressora_id": {
                    "type": "integer"
                },
                "percentual_comissao": {
                    "type": "number"
                },
                "situacao": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                },
                "visivel_frente_caixa": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoGrupoResponse": {
            "type": "object",
            "properties": {
                "agenda": {
                    "type": "integer"
                },
                "agenda_label": {
                    "type": "string"
                },
                "controle_lote": {
                    "type": "integer"
                },
                "controle_lote_label": {
                    "type": "string"
                },
                "cor": {
                    "type": "string"
                },
                "created_at": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "id": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "impressora_id": {
                    "type": "integer"
                },
                "percentual_comissao": {
                    "type": "number"
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                },
                "visivel_frente_caixa": {
                    "type": "integer"
                },
                "visivel_frente_caixa_label": {
                    "type": "string"
                }
            }
        },
        "dto.ProdutoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.ProdutoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoMarcaListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.ProdutoMarcaResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoMarcaRequest": {
            "type": "object",
            "required": [
                "descricao"
            ],
            "properties": {
                "descricao": {
                    "type": "string",
                    "maxLength": 255
                },
                "situacao": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoMarcaResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoModeloListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.ProdutoModeloResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoModeloRequest": {
            "type": "object",
            "required": [
                "descricao"
            ],
            "properties": {
                "descricao": {
                    "type": "string",
                    "maxLength": 255
                },
                "situacao": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoModeloResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoRequest": {
            "type": "object",
            "required": [
                "codigo",
                "empresa_filial_id",
                "ncm_numero",
                "nome",
                "resumo",
                "tipo_produto_id"
            ],
            "properties": {
                "altura": {
                    "type": "number"
                },
                "codigo": {
                    "type": "integer"
                },
                "codigo_barras": {
                    "type": "string"
                },
                "comprimento": {
                    "type": "number"
                },
                "created_by": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "integer"
                },
                "custo_compra": {
                    "type": "number"
                },
                "desconto_maximo": {
                    "type": "number"
                },
                "descricao": {
                    "type": "string"
                },
                "empresa_filial_id": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "especie_id": {
                    "type": "integer"
                },
                "estoque_minimo": {
                    "description": "============================================================\nESTOQUE\n============================================================",
                    "type": "number"
                },
                "largura": {
                    "type": "number"
                },
                "lote_economico": {
                    "type": "number"
                },
                "marca_id": {
                    "type": "integer"
                },
                "modelo_id": {
                    "type": "integer"
                },
                "ncm_numero": {
                    "type": "integer"
                },
                "nome": {
                    "type": "string"
                },
                "peso_bruto": {
                    "description": "============================================================\nDIMENSÕES E PESOS\n============================================================",
                    "type": "number"
                },
                "peso_liquido": {
                    "type": "number"
                },
                "produto_grupo_id": {
                    "type": "integer"
                },
                "produto_subgrupo_id": {
                    "type": "integer"
                },
                "referencia": {
                    "description": "============================================================\nREFERÊNCIAS E DESCRIÇÃO\n============================================================",
                    "type": "string"
                },
                "referencia2": {
                    "type": "string"
                },
                "referencia3": {
                    "type": "string"
                },
                "referencia4": {
                    "type": "string"
                },
                "resumo": {
                    "type": "string"
                },
                "serie_id": {
                    "type": "integer"
                },
                "situacao": {
                    "description": "1-Ativo, 2-Inativo, 3-Bloqueado",
                    "type": "integer"
                },
                "tipo_produto_id": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoResponse": {
            "type": "object",
            "properties": {
                "altura": {
                    "type": "number"
                },
                "codigo": {
                    "type": "integer"
                },
                "codigo_barras": {
                    "type": "string"
                },
                "comprimento": {
                    "type": "number"
                },
                "created_at": {
                    "description": "============================================================\nAUDITORIA\n============================================================",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "custo_compra": {
                    "type": "number"
                },
                "data_alteracao": {
                    "type": "string"
                },
                "desconto_maximo": {
                    "type": "number"
                },
                "descricao": {
                    "type": "string"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "empresa_filial_nome": {
                    "type": "string"
                },
                "especie_id": {
                    "type": "integer"
                },
                "especie_nome": {
                    "type": "string"
                },
                "estoque_minimo": {
                    "description": "============================================================\nESTOQUE\n============================================================",
                    "type": "number"
                },
                "id": {
                    "description": "============================================================\nDADOS PRINCIPAIS\n============================================================",
                    "type": "integer"
                },
                "largura": {
                    "type": "number"
                },
                "lote_economico": {
                    "type": "number"
                },
                "marca_id": {
                    "type": "integer"
                },
                "marca_nome": {
                    "type": "string"
                },
                "modelo_id": {
                    "type": "integer"
                },
                "modelo_nome": {
                    "type": "string"
                },
                "ncm_descricao": {
                    "type": "string"
                },
                "ncm_numero": {
                    "type": "integer"
                },
                "nome": {
                    "type": "string"
                },
                "peso_bruto": {
                    "description": "============================================================\nDIMENSÕES E PESOS\n============================================================",
                    "type": "number"
                },
                "peso_liquido": {
                    "type": "number"
                },
                "produto_grupo_id": {
                    "type": "integer"
                },
                "produto_grupo_nome": {
                    "type": "string"
                },
                "produto_subgrupo_id": {
                    "type": "integer"
                },
                "produto_subgrupo_nome": {
                    "type": "string"
                },
                "referencia": {
                    "description": "============================================================\nREFERÊNCIAS E DESCRIÇÃO\n============================================================",
                    "type": "string"
                },
                "referencia2": {
                    "type": "string"
                },
                "referencia3": {
                    "type": "string"
                },
                "referencia4": {
                    "type": "string"
                },
                "resumo": {
                    "type": "string"
                },
                "saldo_estoque": {
                    "type": "number"
                },
                "serie_id": {
                    "type": "integer"
                },
                "serie_nome": {
                    "type": "string"
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "tipo_produto_id": {
                    "type": "integer"
                },
                "tipo_produto_nome": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoSubgrupoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.ProdutoSubgrupoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoSubgrupoRequest": {
            "type": "object",
            "required": [
                "descricao"
            ],
            "properties": {
                "descricao": {
                    "type": "string",
                    "maxLength": 255
                },
                "situacao": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoSubgrupoResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoTamanhoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.ProdutoTamanhoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoTamanhoRequest": {
            "type": "object",
            "required": [
                "descricao",
                "empresa_filial_id"
            ],
            "properties": {
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string",
                    "maxLength": 255
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "id": {
                    "type": "integer"
                },
                "sigla": {
                    "type": "string",
                    "maxLength": 10
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoTamanhoResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "descricao": {
                    "type": "string"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "empresa_filial_nome": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "sigla": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoVariacaoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.ProdutoVariacaoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoVariacaoRequest": {
            "type": "object",
            "required": [
                "empresa_filial_id",
                "produto_id",
                "sku"
            ],
            "properties": {
                "cor_id": {
                    "type": "integer"
                },
                "created_by": {
                    "description": "AUDITORIA",
                    "type": "integer"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "estoque_atual": {
                    "description": "Para o estoque inicial",
                    "type": "number"
                },
                "id": {
                    "description": "CAMPOS PRINCIPAIS",
                    "type": "integer"
                },
                "preco_adicional": {
                    "type": "number"
                },
                "produto_id": {
                    "type": "integer"
                },
                "sku": {
                    "type": "string",
                    "maxLength": 50
                },
                "tamanho_id": {
                    "type": "integer"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.ProdutoVariacaoResponse": {
            "type": "object",
            "properties": {
                "cor_id": {
                    "type": "integer"
                },
                "cor_nome": {
                    "type": "string"
                },
                "cor_sigla": {
                    "type": "string"
                },
                "created_at": {
                    "description": "AUDITORIA",
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "deleted_at": {
                    "type": "string"
                },
                "empresa_filial_id": {
                    "type": "integer"
                },
                "empresa_filial_nome": {
                    "type": "string"
                },
                "estoque_atual": {
                    "type": "number"
                },
                "id": {
                    "description": "DADOS PRINCIPAIS",
                    "type": "integer"
                },
                "preco_adicional": {
                    "type": "number"
                },
                "produto_id": {
                    "type": "integer"
                },
                "produto_nome": {
                    "description": "RELACIONAMENTOS (apenas IDs e Nomes para evitar recursão)",
                    "type": "string"
                },
                "sku": {
                    "type": "string"
                },
                "tamanho_id": {
                    "type": "integer"
                },
                "tamanho_nome": {
                    "type": "string"
                },
                "tamanho_sigla": {
                    "type": "string"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.RefreshTokenRequest": {
            "type": "object",
            "required": [
                "refresh_token"
            ],
            "properties": {
                "refresh_token": {
                    "type": "string"
                }
            }
        },
        "dto.RefreshTokenResponse": {
            "type": "object",
            "properties": {
                "access_token": {
                    "type": "string"
                },
                "expires_in": {
                    "type": "integer"
                },
                "token_type": {
                    "type": "string"
                }
            }
        },
        "dto.TabelaPrecoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.TabelaPrecoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.TabelaPrecoProdutoListResponse": {
            "type": "object",
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/dto.TabelaPrecoProdutoResponse"
                    }
                },
                "limit": {
                    "type": "integer"
                },
                "page": {
                    "type": "integer"
                },
                "total": {
                    "type": "integer"
                },
                "total_pages": {
                    "type": "integer"
                }
            }
        },
        "dto.TabelaPrecoProdutoRequest": {
            "type": "object",
            "required": [
                "produto_id",
                "valor_minimo",
                "valor_padrao"
            ],
            "properties": {
                "margem_lucro": {
                    "type": "number"
                },
                "perc_desconto_atacado": {
                    "type": "number"
                },
                "produto_id": {
                    "type": "integer"
                },
                "quant_atacado": {
                    "type": "number"
                },
                "situacao": {
                    "type": "integer"
                },
                "valor_custo": {
                    "type": "number"
                },
                "valor_minimo": {
                    "type": "number",
                    "minimum": 0
                },
                "valor_padrao": {
                    "type": "number",
                    "minimum": 0
                }
            }
        },
        "dto.TabelaPrecoProdutoResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "item": {
                    "type": "integer"
                },
                "margem_lucro": {
                    "type": "number"
                },
                "perc_desconto_atacado": {
                    "type": "number"
                },
                "produto_id": {
                    "type": "integer"
                },
                "produto_nome": {
                    "type": "string"
                },
                "quant_atacado": {
                    "type": "number"
                },
                "situacao": {
                    "type": "integer"
                },
                "situacao_label": {
                    "type": "string"
                },
                "tabela_preco_id": {
                    "type": "integer"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                },
                "valor_custo": {
                    "type": "number"
                },
                "valor_minimo": {
                    "type": "number"
                },
                "valor_padrao": {
                    "type": "number"
                }
            }
        },
        "dto.TabelaPrecoRequest": {
            "type": "object",
            "required": [
                "data_inicio",
                "descricao"
            ],
            "properties": {
                "data_fim": {
                    "description": "Formato: YYYY-MM-DD",
                    "type": "string"
                },
                "data_inicio": {
                    "description": "Formato: YYYY-MM-DD",
                    "type": "string"
                },
                "descricao": {
                    "type": "string",
                    "maxLength": 100
                },
                "percentual": {
                    "type": "number"
                },
                "percentual_servico": {
                    "type": "number"
                },
                "tipo": {
                    "type": "integer"
                },
                "tipo_servico": {
                    "type": "integer"
                }
            }
        },
        "dto.TabelaPrecoResponse": {
            "type": "object",
            "properties": {
                "created_at": {
                    "type": "string"
                },
                "created_by": {
                    "type": "integer"
                },
                "data_fim": {
                    "type": "string"
                },
                "data_inicio": {
                    "type": "string"
                },
                "descricao": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "percentual": {
                    "type": "number"
                },
                "percentual_servico": {
                    "type": "number"
                },
                "tipo": {
                    "type": "integer"
                },
                "tipo_servico": {
                    "type": "integer"
                },
                "updated_at": {
                    "type": "string"
                },
                "updated_by": {
                    "type": "integer"
                }
            }
        },
        "dto.UsuarioInfo": {
            "type": "object",
            "properties": {
                "empresa_id": {
                    "type": "integer"
                },
                "empresa_nome": {
                    "type": "string"
                },
                "grupo_id": {
                    "type": "integer"
                },
                "grupo_nome": {
                    "type": "string"
                },
                "id": {
                    "type": "integer"
                },
                "login": {
                    "type": "string"
                },
                "nome": {
                    "type": "string"
                }
            }
        },
        "utils.ErrorResponse": {
            "type": "object",
            "properties": {
                "error": {
                    "type": "string"
                },
                "message": {
                    "type": "string"
                }
            }
        }
    }
}`

// SwaggerInfo holds exported Swagger Info so clients can modify it
var SwaggerInfo = &swag.Spec{
	Version:          "",
	Host:             "",
	BasePath:         "",
	Schemes:          []string{},
	Title:            "",
	Description:      "",
	InfoInstanceName: "swagger",
	SwaggerTemplate:  docTemplate,
	LeftDelim:        "{{",
	RightDelim:       "}}",
}

func init() {
	swag.Register(SwaggerInfo.InstanceName(), SwaggerInfo)
}
